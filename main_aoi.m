...
M = 5;%M+1 is the size of NOMA group
...
P = 1; %transmit SNR, or the transmit power budget
ct = 5000;% number of super-frames
...
aoi_sim = T+ mean(yj.^2)/mean(yj);
aoi_td = T+ mean(yj_td.^2)/mean(yj_td);
...
for q = 0 : 1000
    for m = testm
        for n = 1 : M+1
            sum1 = sum1 + ((M+1-m)*N*T+q*N*T+n*N*T)...
                *PEjm(m)*PEjm(n)*(Pxj1)^q*...
                prod(tempp(m+1:M+1))*prod(tempp(1:n-1));
        end
    end
